...
**Response**: Gentoo has greater median than chinstrap and adelie. we can observe that adelie has outliers.
...
**Response**: In Boxplot we can easily find out outliers where as in jitter plot we can not differentiate between outliers and the normal data points.
...
**Response**: activity02-ggplot2-intro.md looks more professional and easier to read.
...
Play around with doing the  boxplot laid over the jitterplot and the boxplot laid over the jitterplot.
Which do you prefer?
Why?

**Response**: I do pefer the jitterplot laid over the boxplot because we can see all points clearly
...
**Response**: Having all of them mapped in the ggplot call gives color to both boxplot and jitterplot whereas mapping aesthetics to geom_boxplot gives color only to jitterplot
